...
install() {
    bash <(curl -Ls https://raw.githubusercontent.com/mohammad7747/3x-ui/main/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
update_shell() {
    wget -O /usr/bin/x-ui -N --no-check-certificate https://github.com/mohammad7747/3x-ui/raw/main/x-ui.sh
    if [[ $? != 0 ]]; then
        echo ""
